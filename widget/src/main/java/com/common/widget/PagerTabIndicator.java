 ...
 * @version 1.1
 */
public class PagerTabIndicator extends HorizontalScrollView implements
    ...
    private boolean allowFullWidth; // 内容宽度无法充满时，允许自动调整Item的宽度以充满
    ...
    private Drawable slidingBlockTxtColor;
    ...
    public PagerTabIndicator(Context context, AttributeSet attrs) {
        ...
        if (attrs != null) {
            TypedArray attrsTypedArray = context.obtainStyledAttributes(attrs, R.styleable.PagerTabIndicator);
            if (attrsTypedArray != null) {
                allowFullWidth = attrsTypedArray.getBoolean(R.styleable.PagerTabIndicator_fullWidth, false);
                slidingBlockDrawable = attrsTypedArray.getDrawable(R.styleable.PagerTabIndicator_slidingBlock);
                slidingBlockTxtColor = attrsTypedArray.getDrawable(R.styleable.PagerTabIndicator_slidingBlockTxtColor);
                disableViewPager = attrsTypedArray.getBoolean(R.styleable.PagerTabIndicator_disableViewPager, false);
    ...
    public View getTitleLayout(int index) {
        return getTabsLayout().getChildAt(index);//返回Tab容器
    }
    ...
    public View getBadgeView(int i) {
        return getTabsLayout().getChildAt(i).findViewById(setBadgeViewId());
    }

    @IdRes
    public int setBadgeViewId() {
        return 0;
    }
     ...
     * 设置是否充满屏幕
     *
     * @param allowFullWidth true：当内容的宽度无法充满屏幕时，自动调整每一个Item的宽度以充满屏幕
     */
    public void setAllowFullWidth(boolean allowFullWidth) {
